...
/***/ function(module, exports, __webpack_require__) {
	...
	function getVersion(element) {
	  var version = $(element).attr('data-kotlin-version');
	...
	function updateState(state) {
	  localStorage.setItem("apiState", (0, _stringify2.default)(state));
	  var $platformDependentElements = $('[data-platform]');
	  var $versionDependentElements = $('[data-kotlin-version]');
	  $versionDependentElements.removeClass('hidden');
	  $platformDependentElements.removeClass('hidden');

	  if (state.platform.toLowerCase() != 'all') {
	    $platformDependentElements.each(function (ind, element) {
	      var $element = $(element);
	  ...
	  $versionDependentElements.each(function (ind, element) {
	    var $element = $(element);
	...
	function addSelectToPanel(panelElement, title, config) {
	  var selectElement = $('<div class="api-panel__select"><span class="api-panel__dropdown-title">' + title + '</span></div>');
	  $(panelElement).append(selectElement);
	  new Dropdown(selectElement, config);
	}

	function initializeSelects() {
	  var $breadcrumbs = $('.api-docs-breadcrumbs');
	  if ($breadcrumbs.length > 0) {
	    $breadcrumbs.wrap('<div class="api-page-panel"></div>').after('<div class="api-panel__switchers"></div>');
	  } else {
	    $('.page-content').prepend('<div class="api-page-panel"><div class="api-docs-breadcrumbs"></div><div class="api-panel__switchers"></div></div>');
	  }

	  var switchersPanel = $('.api-panel__switchers')[0];

	  var state = localStorage.getItem("apiState") ? JSON.parse(localStorage.getItem("apiState")) : {
	    platform: 'all',
	    version: '1.1'
	  };
	  updateState(state);
	  ...
	  addSelectToPanel(switchersPanel, "Platform", {
	    ...
	    onSelect: function onSelect(platform) {
	  ...
	  addSelectToPanel(switchersPanel, "Version", {
	    ...
	    onSelect: function onSelect(version) {
	...
	function addTag(rowElement, tag) {
	  var $tagsElement = $(rowElement).find('.tags');
	...
	function addTags() {
	  ...
	  $('[data-kotlin-version]').each(function (ind, element) {
	    return addTag(element, getVersion(element));
	  });

	  $('[data-jre-version]').each(function (ind, element) {
	    var platform = element.getAttribute('data-jre-version');
	    addTag(element, platform);
	  });
	}
